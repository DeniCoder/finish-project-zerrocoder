...
async def add_income_amount(message: types.Message, state: FSMContext):
    ...
    # !!! Берём только категории ДОХОДОВ
    categories = await sync_to_async(list)(Category.objects.filter(is_income=True))
    if not categories:
        await message.answer("Категории доходов ещё не созданы. Добавьте категории через админку Django.")
        await state.clear()
        return
...
async def add_income_category(message: types.Message, state: FSMContext):
    try:
        category_id = int(message.text)
        category = await sync_to_async(Category.objects.get)(id=category_id, is_income=True)
        await state.update_data(category_id=category_id)
    except (ValueError, Category.DoesNotExist):
        await message.answer("Некорректный номер категории. Попробуйте ещё раз.")
        return
...
@dp.message(AddIncomeStates.waiting_for_date)
